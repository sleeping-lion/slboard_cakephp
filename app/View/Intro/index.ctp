<?php $this -> Html -> script(array('plugin/jquery.easing.1.3.pack.js','plugin/jquery.tools.min.js','plugin/jquery.fancybox.1.3.4.js','plugin/jquery.uri.js','intro/index.js'), array('inline' => false)); ?>
...
<section id="sl_intro_index">
  ...
  <article id="intro_main_content" itemscope itemtype="http://schema.org/Person">
    ...
    <div id="sl_intro_left">
		<?php echo $this -> Html -> link($this->Html->image('/files/user_photo/photo/'.$userPhoto['UserPhoto']['id'].'/small_'.$userPhoto['UserPhoto']['photo'], array('alt' =>$user['User']['name'])), array('action' => 'view',$userPhoto['UserPhoto']['id']),array('escape'=>false,'class'=>'simple_image')) ?>
    </div>
    <?php endif ?>
